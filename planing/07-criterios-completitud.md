...
- [ ] `pnpm build` compila los 3 packages sin errores de TypeScript
...
## Servidor — Agente (loop ReAct)

- [ ] `POST /api/sessions/:id/messages` con `{ content: "..." }` inicia el loop del agente
- [ ] La respuesta es un SSE stream con eventos tipados: `token`, `tool_call`, `tool_result`, `permission_request`, `done`, `error`
...
- [ ] En modo `build`, los tools se ejecutan sin pedir permiso
...
- [ ] Todos los mensajes (user, assistant, tool_call, tool_result) se persisten en la tabla `messages`
...
- [ ] Los mensajes antiguos se resumen en un mensaje de tipo `system` que reemplaza los originales
...
- [ ] `bash`: usa shell apropiada según el OS (powershell/cmd en Windows, bash/sh en Linux)
- [ ] `list_files`: respeta `maxDepth` y excluye `node_modules`/`.git` por defecto
...
- [ ] `sendMessage()` retorna un `AsyncIterable` que emite eventos del agente
...
- [ ] Muestra la lista de modelos descargados con sus tamaños
...
- [ ] La respuesta del agente aparece token por token (streaming visible)
- [ ] Cuando el agente hace un tool call, se muestra: nombre del tool, argumentos, y resultado
...
- [ ] El tool `bash` funciona en Windows (usa PowerShell) y Linux (usa bash)
